...
CIMGUI_API void   c_iam_set_imgui_allocator_functions(ImGuiMemAllocFunc alloc_func, ImGuiMemFreeFunc free_func, void* user_data) { ImGui::SetAllocatorFunctions(alloc_func, free_func, user_data); }
...
CIMGUI_API void   c_iam_pool_clear() { iam_pool_clear(); }
...
CIMGUI_API void   c_iam_gradient_add(iam_gradient* self, float position, ImVec4* color) { self->add(position, *color); }
CIMGUI_API void   c_iam_gradient_sample(ImVec4* pOut, iam_gradient* self, float t, int color_space) { *pOut = self->sample(t, color_space); }
...
CIMGUI_API void   c_iam_transform_apply(ImVec2* pOut, iam_transform* transform, ImVec2* point) { *pOut = transform->apply(*point); }
CIMGUI_API void   c_iam_transform_inverse(iam_transform* pOut, iam_transform* transform) { *pOut = transform->inverse(); }
CIMGUI_API void   c_iam_transform_multiply(iam_transform* pOut, iam_transform* transform, iam_transform* other) { *pOut = *transform * *other; }
...
CIMGUI_API void   c_iam_instance_destroy(iam_instance* self) { self->destroy(); }
...
CIMGUI_API void       c_iam_set_config_directory(const char* path) { iam_set_config_directory(path); }
